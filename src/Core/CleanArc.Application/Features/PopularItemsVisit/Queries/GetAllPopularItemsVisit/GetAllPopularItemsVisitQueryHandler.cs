    ...
    internal class GetAllPopularItemsVisitQueryHandler : IRequestHandler<GetAllPopularItemsVisitQuery, OperationResult<List<GetAllPopularItemsVisitQueryResult>>>
    {
        ...
        public async ValueTask<OperationResult<List<GetAllPopularItemsVisitQueryResult>>> Handle(GetAllPopularItemsVisitQuery request, CancellationToken cancellationToken)
        {
            using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
            {
                ...
                var response = await _unitOfWork.PopularItemsVisitRepository.GetAllAsync(request.searchRequest);

                if (response.Code != 200)
                {
                    return OperationResult<List<GetAllPopularItemsVisitQueryResult>>.FailureResult(
                        response.Message,
                    response.Code
                    );
                }

                var mappedResult = _mapper.Map<List<GetAllPopularItemsVisitQueryResult>>(response.Data);
                (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(mappedResult);

                return OperationResult<List<GetAllPopularItemsVisitQueryResult>>.SuccessResult(
                    mappedResult,
                    response.Code,
                    response.Message
                );
