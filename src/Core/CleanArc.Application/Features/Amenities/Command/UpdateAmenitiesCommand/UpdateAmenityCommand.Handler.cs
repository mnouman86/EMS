...
internal class UpdateAmenityCommandHandler : IRequestHandler<UpdateAmenityCommand, OperationResult<bool>>
{
    ...
    public async ValueTask<OperationResult<bool>> Handle(UpdateAmenityCommand request, CancellationToken cancellationToken)
    {
        using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
        {
            ...
            { UpdatedBy = user.Id, ID = request.ID, Description = request.Description, Name = request.Name,CategoryID=request.CategoryID ,Icon=request.Icon,ServiceCategoryID=request.ServiceCategoryID});
