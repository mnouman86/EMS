    ...
    internal class GetAllActivityIncludedOptionQueryHandler : IRequestHandler<GetAllActivityIncludedOptionQuery, OperationResult<List<GetAllActivityIncludedOptionQueryResult>>>
    {
        ...
        public async ValueTask<OperationResult<List<GetAllActivityIncludedOptionQueryResult>>> Handle(GetAllActivityIncludedOptionQuery request, CancellationToken cancellationToken)
        {
            using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
            {
                ...
                var response = await _unitOfWork.ActivityIncludedOptionRepository.GetAllAsync(request.searchRequest);

                if (response.Code != 200)
                {
                    return OperationResult<List<GetAllActivityIncludedOptionQueryResult>>.FailureResult(
                        response.Message,
                    response.Code
                    );
                }

                var mappedResult = _mapper.Map<List<GetAllActivityIncludedOptionQueryResult>>(response.Data);
                (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(mappedResult);

                return OperationResult<List<GetAllActivityIncludedOptionQueryResult>>.SuccessResult(
                    mappedResult,
                    response.Code,
                    response.Message,
                    response.TotalCount
