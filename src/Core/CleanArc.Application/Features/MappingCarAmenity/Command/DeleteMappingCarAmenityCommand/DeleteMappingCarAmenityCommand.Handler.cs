...
internal class DeleteMappingCarAmenityCommandHandler : IRequestHandler<DeleteMappingCarAmenityCommand, OperationResult<ResponseEntity>>
{
    ...
    public async ValueTask<OperationResult<ResponseEntity>> Handle(DeleteMappingCarAmenityCommand request, CancellationToken cancellationToken)
    {
        using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
        {
            ...
            var result = await _unitOfWork.MappingCarAmenityRepository.DeleteAsync(request.SelectedIds, user.Id, request.CultureId);
            await _unitOfWork.CommitAsync();
            //  (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(true);
            return OperationResult<ResponseEntity>.SuccessResult(result);
        }
