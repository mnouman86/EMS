...
namespace CleanArc.Application.Features.Country.Command.UpdateCountryCommand
{
    internal class UpdateCountryCommandHandler : IRequestHandler<UpdateCountryCommand, OperationResult<ResponseEntity>>
    {
        ...
        public UpdateCountryCommandHandler(IUnitOfWork unitOfWork, IAppUserManager userManager, IConfiguration configuration, IMapper mapper, ILogger<UpdateCountryCommandHandler> logger, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async ValueTask<OperationResult<ResponseEntity>> Handle(UpdateCountryCommand request, CancellationToken cancellationToken)
        {
            using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
            {
                ...
                { UpdatedBy = user.Id, Id = request.Id, Description = request.Description, Nationality=request.Nationality, Name = request.Name,CultureId=request.CultureId });
                await _unitOfWork.CommitAsync();
                (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(true);
                return OperationResult<ResponseEntity>.SuccessResult(result);
            }
