...
namespace CleanArc.Application.Features.KBInterested.Queries.GetKBInterestedById
{
    internal class GetKBInterestedByIdQueryHandler : IRequestHandler<GetKBInterestedByIdQuery, OperationResult<GetKBInterestedByIdQueryResult>>
    {
        ...
        public async ValueTask<OperationResult<GetKBInterestedByIdQueryResult>> Handle(GetKBInterestedByIdQuery request, CancellationToken cancellationToken)
        {
            ...
            {
                //var KBInterested = await _unitOfWork.KBInterestedRepository.GetByIdAsync(request.searchRequestById);
                ...
                var response = await _unitOfWork.KBInterestedRepository.GetByIdAsync(request.searchRequestById);

                if (response.Code != 200)
                {
                    return OperationResult<GetKBInterestedByIdQueryResult>.FailureResult(
                        response.Message,
                    response.Code
                    );
                }

                var mappedResult = _mapper.Map<GetKBInterestedByIdQueryResult>(response.Data);
                (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(mappedResult);

                return OperationResult<GetKBInterestedByIdQueryResult>.SuccessResult(
                    mappedResult,
                    response.Code,
                    response.Message
                );
