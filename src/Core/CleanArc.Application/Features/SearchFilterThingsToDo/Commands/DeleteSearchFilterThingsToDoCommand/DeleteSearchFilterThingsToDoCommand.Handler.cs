...
internal class DeleteSearchFilterThingsToDoCommandHandler: IRequestHandler<DeleteSearchFilterThingsToDoCommand, OperationResult<ResponseEntity>>
{
    ...
    private readonly ILogger<DeleteSearchFilterThingsToDoCommandHandler> _logger;
    private readonly IHttpContextAccessor _httpContextAccessor; // Add IHttpContextAccessor
    //private readonly IUnitOfWork _unitOfWork;
    //private readonly IAppUserManager _userManager;


    public DeleteSearchFilterThingsToDoCommandHandler(IUnitOfWork unitOfWork, IAppUserManager userManager, IConfiguration configuration, IMapper mapper, ILogger<DeleteSearchFilterThingsToDoCommandHandler> logger, IHttpContextAccessor httpContextAccessor)
    {
    ...
    public async ValueTask<OperationResult<ResponseEntity>> Handle(DeleteSearchFilterThingsToDoCommand request, CancellationToken cancellationToken)
    {
        using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
        {
           ...
           // await _unitOfWork.SearchFilterThingsToDoRepository.DeleteAsync(request.deleteRequest, user.Id);
            await _unitOfWork.CommitAsync();
          //  (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(true);
