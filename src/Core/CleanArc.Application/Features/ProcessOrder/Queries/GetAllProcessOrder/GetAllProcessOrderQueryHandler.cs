    ...
    internal class GetAllProcessOrderQueryHandler : IRequestHandler<GetAllProcessOrderQuery, OperationResult<List<GetAllProcessOrderQueryResult>>>
    {
        ...
        public async ValueTask<OperationResult<List<GetAllProcessOrderQueryResult>>> Handle(GetAllProcessOrderQuery request, CancellationToken cancellationToken)
        {
            using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
            {
                ...
                var response = await _unitOfWork.ProcessOrderRepository.GetAllAsync(request.searchRequest);

                if (response.Code != 200)
                {
                    return OperationResult<List<GetAllProcessOrderQueryResult>>.FailureResult(
                        response.Message,
                    response.Code
                    );
                }

                var mappedResult = _mapper.Map<List<GetAllProcessOrderQueryResult>>(response.Data);
                (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(mappedResult);

                return OperationResult<List<GetAllProcessOrderQueryResult>>.SuccessResult(
                    mappedResult,
                    response.Code,
                    response.Message,
                    response.TotalCount
