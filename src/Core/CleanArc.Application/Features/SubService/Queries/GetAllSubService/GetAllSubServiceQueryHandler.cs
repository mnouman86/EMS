...
namespace CleanArc.Application.Features.SubService.Queries.GetAllSubService
{
    internal class GetAllSubServiceQueryHandler : IRequestHandler<GetAllSubServiceQuery, OperationResult<List<GetAllSubServiceQueryResult>>>
    {
        ...
        public async ValueTask<OperationResult<List<GetAllSubServiceQueryResult>>> Handle(GetAllSubServiceQuery request, CancellationToken cancellationToken)
        {
            ...
            {
                var response = await _unitOfWork.SubServiceRepository.GetAllAsync(request.searchRequest);
                ...
                {
                    return OperationResult<List<GetAllSubServiceQueryResult>>.FailureResult(
                        response.Message,
                    response.Code
                    );
                }

                var mappedResult = _mapper.Map<List<GetAllSubServiceQueryResult>>(response.Data);
                (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(mappedResult);

                return OperationResult<List<GetAllSubServiceQueryResult>>.SuccessResult(
                    mappedResult,
                    response.Code,
                    response.Message
                );
