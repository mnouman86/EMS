...
namespace CleanArc.Application.Features.CarImage.Command.DeleteCarImageCommand
{
    internal class DeleteCarImageCommandHandler : IRequestHandler<DeleteCarImageCommand, OperationResult<bool>>
    {
        ...
        private readonly ILogger<DeleteCarImageCommandHandler> _logger;
        private readonly IHttpContextAccessor _httpContextAccessor; // Add IHttpContextAccessor
                                                                    //private readonly IUnitOfWork _unitOfWork;
                                                                    //private readonly IAppUserManager _userManager;


        public DeleteCarImageCommandHandler(IUnitOfWork unitOfWork, IAppUserManager userManager, IConfiguration configuration, IMapper mapper, ILogger<DeleteCarImageCommandHandler> logger, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async ValueTask<OperationResult<bool>> Handle(DeleteCarImageCommand request, CancellationToken cancellationToken)
        {
            using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
            {
                ...
                await _unitOfWork.CarImageRepository.DeleteAsync(request.SelectedIds, user.Id);
