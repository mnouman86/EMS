...
internal class CreateActivityAddressCommandHandler: IRequestHandler<CreateGenericAddressCommand, OperationResult<ResponseEntity>>
{
    ...
    public async ValueTask<OperationResult<ResponseEntity>> Handle(CreateGenericAddressCommand request, CancellationToken cancellationToken)
    {
        using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
        {
           ...
           var result= await _unitOfWork.ActivityAddressRepository.AddAsync(new Domain.Entities.GenericAddress.GenericAddress()
            { CreatedBy = user.Id,
