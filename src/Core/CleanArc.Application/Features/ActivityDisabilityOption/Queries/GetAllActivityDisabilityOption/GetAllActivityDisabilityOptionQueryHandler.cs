    ...
    internal class GetAllActivityDisabilityOptionQueryHandler : IRequestHandler<GetAllActivityDisabilityOptionQuery, OperationResult<List<GetAllActivityDisabilityOptionQueryResult>>>
    {
        ...
        public async ValueTask<OperationResult<List<GetAllActivityDisabilityOptionQueryResult>>> Handle(GetAllActivityDisabilityOptionQuery request, CancellationToken cancellationToken)
        {
            using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
            {
                ...
                var response = await _unitOfWork.ActivityDisabilityOptionRepository.GetAllAsync(request.searchRequest);

                if (response.Code != 200)
                {
                    return OperationResult<List<GetAllActivityDisabilityOptionQueryResult>>.FailureResult(
                        response.Message,
                    response.Code
                    );
                }

                var mappedResult = _mapper.Map<List<GetAllActivityDisabilityOptionQueryResult>>(response.Data);
                (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(mappedResult);

                return OperationResult<List<GetAllActivityDisabilityOptionQueryResult>>.SuccessResult(
                    mappedResult,
                    response.Code,
                    response.Message,
                    response.TotalCount
