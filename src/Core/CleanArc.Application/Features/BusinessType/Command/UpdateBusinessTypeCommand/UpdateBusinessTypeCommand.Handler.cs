...
{
    internal class UpdateBusinessTypeCommandHandler : IRequestHandler<UpdateBusinessTypeCommand, OperationResult<ResponseEntity>>
    {
        ...
        public async ValueTask<OperationResult<ResponseEntity>> Handle(UpdateBusinessTypeCommand request, CancellationToken cancellationToken)
        {
            using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
            {
                ...
                var result = await _unitOfWork.BusinessTypeRepository.UpdateAsync(new Domain.Entities.BusinessType.BusinessType()
