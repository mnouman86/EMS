...
internal class CreateEmployeeCommandHandler : IRequestHandler<CreateEmployeeCommand, OperationResult<ResponseEntity>>
{
    ...
    public async ValueTask<OperationResult<ResponseEntity>> Handle(CreateEmployeeCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = await _unitOfWork.EmployeeRepository.AddAsync(entity);
