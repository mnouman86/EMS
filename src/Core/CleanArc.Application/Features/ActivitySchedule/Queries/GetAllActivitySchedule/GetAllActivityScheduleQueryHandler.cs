    ...
    internal class GetAllActivityScheduleQueryHandler : IRequestHandler<GetAllActivityScheduleQuery, OperationResult<List<GetAllActivityScheduleQueryResult>>>
    {
        ...
        public async ValueTask<OperationResult<List<GetAllActivityScheduleQueryResult>>> Handle(GetAllActivityScheduleQuery request, CancellationToken cancellationToken)
        {
            using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
            {
                ...
                var response = await _unitOfWork.ActivityScheduleRepository.GetAllAsync(request.searchRequest);

                if (response.Code != 200)
                {
                    return OperationResult<List<GetAllActivityScheduleQueryResult>>.FailureResult(
                        response.Message,
                    response.Code
                    );
                }

                var mappedResult = _mapper.Map<List<GetAllActivityScheduleQueryResult>>(response.Data);
                (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(mappedResult);

                return OperationResult<List<GetAllActivityScheduleQueryResult>>.SuccessResult(
                    mappedResult,
                    response.Code,
                    response.Message
                );
