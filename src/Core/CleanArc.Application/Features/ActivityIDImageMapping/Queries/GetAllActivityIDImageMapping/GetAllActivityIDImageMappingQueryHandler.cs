    ...
    internal class GetAllActivityIDImageMappingQueryHandler : IRequestHandler<GetAllActivityIDImageMappingQuery, OperationResult<List<GetAllActivityIDImageMappingQueryResult>>>
    {
        ...
        public async ValueTask<OperationResult<List<GetAllActivityIDImageMappingQueryResult>>> Handle(GetAllActivityIDImageMappingQuery request, CancellationToken cancellationToken)
        {
            using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
            {
                ...
                var response = await _unitOfWork.ActivityIDImageMappingRepository.GetAllAsync(request.searchRequest);

                if (response.Code != 200)
                {
                    return OperationResult<List<GetAllActivityIDImageMappingQueryResult>>.FailureResult(
                        response.Message,
                    response.Code
                    );
                }

                var mappedResult = _mapper.Map<List<GetAllActivityIDImageMappingQueryResult>>(response.Data);
                (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(mappedResult);

                return OperationResult<List<GetAllActivityIDImageMappingQueryResult>>.SuccessResult(
                    mappedResult,
                    response.Code,
                    response.Message,
                    response.TotalCount
