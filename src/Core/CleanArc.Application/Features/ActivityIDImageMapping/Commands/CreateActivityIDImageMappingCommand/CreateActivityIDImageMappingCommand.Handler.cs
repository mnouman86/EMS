...
internal class CreateActivityIDImageMappingCommandHandler: IRequestHandler<CreateActivityIDImageMappingCommand, OperationResult<ResponseEntity>>
{
    ...
    private readonly ILogger<CreateActivityIDImageMappingCommandHandler> _logger;
    private readonly IHttpContextAccessor _httpContextAccessor; // Add IHttpContextAccessor
    //private readonly IUnitOfWork _unitOfWork;
    //private readonly IAppUserManager _userManager;


    public CreateActivityIDImageMappingCommandHandler(IUnitOfWork unitOfWork, IAppUserManager userManager, IConfiguration configuration, IMapper mapper, ILogger<CreateActivityIDImageMappingCommandHandler> logger, IHttpContextAccessor httpContextAccessor)
    {
    ...
    public async ValueTask<OperationResult<ResponseEntity>> Handle(CreateActivityIDImageMappingCommand request, CancellationToken cancellationToken)
    {
        using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
        {
            ...
            { CreatedBy = user.Id, ActivityID = request.ActivityID,CultureId = request.CultureId,ImagePaths=request.ImagePaths });
            await _unitOfWork.CommitAsync();
            (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(true);
            return OperationResult<ResponseEntity>.SuccessResult(result);
        }
