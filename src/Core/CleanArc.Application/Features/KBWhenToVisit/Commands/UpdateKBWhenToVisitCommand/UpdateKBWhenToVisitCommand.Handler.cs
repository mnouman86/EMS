...
internal class UpdateKBWhenToVisitCommandHandler:IRequestHandler<UpdateKBWhenToVisitCommand, OperationResult<ResponseEntity>>
{
    ...
    public async ValueTask<OperationResult<ResponseEntity>> Handle(UpdateKBWhenToVisitCommand request, CancellationToken cancellationToken)
    {
        using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
        {
            ...
            var result = await _unitOfWork.KBWhenToVisitRepository.UpdateAsync(new Domain.Entities.KBWhenToVisit.KBWhenToVisit()
