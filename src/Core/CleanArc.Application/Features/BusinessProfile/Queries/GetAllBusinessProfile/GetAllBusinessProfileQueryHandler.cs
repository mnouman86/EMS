...
namespace CleanArc.Application.Features.BusinessProfile.Queries.GetAllBusinessProfile;
    ...
    public async ValueTask<OperationResult<List<GetAllBusinessProfileQueryResult>>> Handle(GetAllBusinessProfileQuery request, CancellationToken cancellationToken)
    {
        using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
        {
           ...
           // request.searchRequest.FilterArray.Add(new FilterParameter { ParameterName = "CreatedBy", ParameterValue = Convert.ToString(userId) });
            ...
            var response = await _unitOfWork.BusinessProfileRepository.GetAllAsync(request.searchRequest);

            if (response.Code != 200)
            {
                return OperationResult<List<GetAllBusinessProfileQueryResult>>.FailureResult(
                    response.Message,
                response.Code
                );
            }

            var mappedResult = _mapper.Map<List<GetAllBusinessProfileQueryResult>>(response.Data);
            (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(mappedResult);

            return OperationResult<List<GetAllBusinessProfileQueryResult>>.SuccessResult(
                mappedResult,
                response.Code,
                response.Message
            );
