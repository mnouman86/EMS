...
internal class DeleteKBDescriptionCommandHandler: IRequestHandler<DeleteKBDescriptionCommand, OperationResult<bool>>
{
    ...
    public async ValueTask<OperationResult<bool>> Handle(DeleteKBDescriptionCommand request, CancellationToken cancellationToken)
    {
        using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
        {
            ...
            await _unitOfWork.KBDescriptionRepository.DeleteAsync(request.SelectedIds, user.Id, request.CultureId);
            await _unitOfWork.CommitAsync();
          //  (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(true);
            return OperationResult<bool>.SuccessResult(true);
        }
