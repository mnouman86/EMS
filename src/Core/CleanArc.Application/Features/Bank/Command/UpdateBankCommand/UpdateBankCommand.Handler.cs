...
namespace CleanArc.Application.Features.Bank.Command.UpdateBankCommand
{
    internal class UpdateBankCommandHandler : IRequestHandler<UpdateBankCommand, OperationResult<ResponseEntity>>
    {
        ...
        public UpdateBankCommandHandler(IUnitOfWork unitOfWork, IAppUserManager userManager, IConfiguration configuration, IMapper mapper, ILogger<UpdateBankCommandHandler> logger, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async ValueTask<OperationResult<ResponseEntity>> Handle(UpdateBankCommand request, CancellationToken cancellationToken)
        {
            using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
            {
                ...
                { UpdatedBy = user.Id, Id = request.Id, Description = request.Description, 
                    CountryLookUpId =request.CountryLookUpId, Name = request.Name,
