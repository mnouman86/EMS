...
internal class DeleteMappingRoomAmenityCommandHandler : IRequestHandler<DeleteMappingRoomAmenityCommand, OperationResult<ResponseEntity>>
{
    ...
    private readonly ILogger<DeleteMappingRoomAmenityCommandHandler> _logger;
    private readonly IHttpContextAccessor _httpContextAccessor; // Add IHttpContextAccessor
                                                                //private readonly IUnitOfWork _unitOfWork;
                                                                //private readonly IAppUserManager _userManager;


    public DeleteMappingRoomAmenityCommandHandler(IUnitOfWork unitOfWork, IAppUserManager userManager, IConfiguration configuration, IMapper mapper, ILogger<DeleteMappingRoomAmenityCommandHandler> logger, IHttpContextAccessor httpContextAccessor)
    {
    ...
    public async ValueTask<OperationResult<ResponseEntity>> Handle(DeleteMappingRoomAmenityCommand request, CancellationToken cancellationToken)
    {
        using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
        {
            ...
            var result = await _unitOfWork.MappingRoomAmenitiesRepository.DeleteAsync(request.deleteRequest, user.Id);
            await _unitOfWork.CommitAsync();
            //  (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(true);
            return OperationResult<ResponseEntity>.SuccessResult(result);
        }
