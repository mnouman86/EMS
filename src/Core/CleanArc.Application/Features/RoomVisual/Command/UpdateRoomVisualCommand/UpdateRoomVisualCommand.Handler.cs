...
internal class UpdateRoomVisualCommandHandler : IRequestHandler<UpdateRoomVisualCommand, OperationResult<bool>>
{
    ...
    public async ValueTask<OperationResult<bool>> Handle(UpdateRoomVisualCommand request, CancellationToken cancellationToken)
    {
        using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
        {
            ...
            await _unitOfWork.RoomVisualRepository.UpdateAsync(new Domain.Entities.RoomVisual.RoomVisual()
            {
                UpdatedBy = user.Id,
                ID = request.ID,
