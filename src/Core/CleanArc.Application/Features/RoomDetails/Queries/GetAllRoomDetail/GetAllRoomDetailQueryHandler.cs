    ...
    internal class GetAllRoomDetailQueryHandler : IRequestHandler<GetAllRoomDetailQuery, OperationResult<List<GetAllRoomDetailQueryResult>>>
    {
        ...
        public async ValueTask<OperationResult<List<GetAllRoomDetailQueryResult>>> Handle(GetAllRoomDetailQuery request, CancellationToken cancellationToken)
        {
            using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
            {
                ...
                var response = await _unitOfWork.RoomDetailsRepository.GetAllAsync(request.searchRequest);

                if (response.Code != 200)
                {
                    return OperationResult<List<GetAllRoomDetailQueryResult>>.FailureResult(
                        response.Message,
                    response.Code
                    );
                }

                var mappedResult = _mapper.Map<List<GetAllRoomDetailQueryResult>>(response.Data);
                (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(mappedResult);

                return OperationResult<List<GetAllRoomDetailQueryResult>>.SuccessResult(
                    mappedResult,
                    response.Code,
                    response.Message
                );
