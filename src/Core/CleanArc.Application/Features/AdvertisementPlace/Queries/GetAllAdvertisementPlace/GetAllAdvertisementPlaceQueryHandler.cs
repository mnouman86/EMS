    ...
    internal class GetAllAdvertisementPlaceQueryHandler : IRequestHandler<GetAllAdvertisementPlaceQuery, OperationResult<List<GetAllAdvertisementPlaceQueryResult>>>
    {
        ...
        public async ValueTask<OperationResult<List<GetAllAdvertisementPlaceQueryResult>>> Handle(GetAllAdvertisementPlaceQuery request, CancellationToken cancellationToken)
        {
            using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
            {
                ...
                var response = await _unitOfWork.AdvertisementPlaceRepository.GetAllAsync(request.searchRequest);

                if (response.Code != 200)
                {
                    return OperationResult<List<GetAllAdvertisementPlaceQueryResult>>.FailureResult(
                        response.Message,
                    response.Code
                    );
                }

                var mappedResult = _mapper.Map<List<GetAllAdvertisementPlaceQueryResult>>(response.Data);
                (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(mappedResult);

                return OperationResult<List<GetAllAdvertisementPlaceQueryResult>>.SuccessResult(
                    mappedResult,
                    response.Code,
                    response.Message
                );
