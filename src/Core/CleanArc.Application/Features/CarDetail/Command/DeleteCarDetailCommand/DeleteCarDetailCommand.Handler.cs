...
{
    internal class DeleteCarDetailCommandHandler : IRequestHandler<DeleteCarDetailCommand, OperationResult<ResponseEntity>>
    {
        ...
        public async ValueTask<OperationResult<ResponseEntity>> Handle(DeleteCarDetailCommand request, CancellationToken cancellationToken)
        {
            using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, request))
            {
                ...
                var result = await _unitOfWork.CarDetailRepository.DeleteAsync(request.SelectedIds, user.Id, request.CultureId);
                await _unitOfWork.CommitAsync();
                //  (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(true);
                return OperationResult<ResponseEntity>.SuccessResult(result);
            }
