...
namespace CleanArc.Application.Features.ActivityDisabilityMapping.Queries.GetActivityDisabilityMappingById
{
    internal class GetActivityDisabilityMappingByIdQueryHandler : IRequestHandler<GetActivityDisabilityMappingByIdQuery, OperationResult<GetActivityDisabilityMappingByIdQueryResult>>
    {
        ...
        public async ValueTask<OperationResult<GetActivityDisabilityMappingByIdQueryResult>> Handle(GetActivityDisabilityMappingByIdQuery request, CancellationToken cancellationToken)
        {
            ...
            {
                //var ActivityDisabilityMapping = await _unitOfWork.ActivityDisabilityMappingRepository.GetByIdAsync(request.searchRequestById);
                ...
                var response = await _unitOfWork.ActivityDisabilityMappingRepository.GetByIdAsync(request.searchRequestById);

                if (response.Code != 200)
                {
                    return OperationResult<GetActivityDisabilityMappingByIdQueryResult>.FailureResult(
                        response.Message,
                    response.Code
                    );
                }

                var mappedResult = _mapper.Map<GetActivityDisabilityMappingByIdQueryResult>(response.Data);
                (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(mappedResult);

                return OperationResult<GetActivityDisabilityMappingByIdQueryResult>.SuccessResult(
                    mappedResult,
                    response.Code,
                    response.Message
                );
