...
public class RoomImagesRepository : IRoomImagesRepository
{
    ...
    /// </summary>
    private readonly ILogger<RoomImagesRepository> _logger;
    ...
    /// 
    public RoomImagesRepository(IConfiguration configuration, IMapper mapper, ILogger<RoomImagesRepository> logger, IHttpContextAccessor httpContextAccessor)
    {
    ...
    public Task<ResponseEntity> AddAsync(RoomImages entity)
    {
    ...
    public async Task<ListResponseWrapper<RoomImages>> GetAllAsync(SearchRequest searchRequest)
    {
        using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, searchRequest))
        {
            using (IDbConnection connection = new SqlConnection(configuration.GetConnectionString("DBConnection1")))
            {
				...
				var response = new ListResponseWrapper<RoomImages> { Data = result.ToList(), TotalCount = parameters.Get<int>("@TotalCount"), Code = parameters.Get<int>("@Code"), Message = parameters.Get<string>("@Message") }; return response;
