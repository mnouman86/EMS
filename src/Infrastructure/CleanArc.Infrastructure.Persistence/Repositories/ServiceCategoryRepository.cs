...
public class ServiceCategoryRepository : IServiceCategoryRepository
{
    ...
    public async Task<string> AddAsync(ServiceCategory serviceCategory)
    {
        using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, serviceCategory))
        {
            using (IDbConnection connection = new SqlConnection(configuration.GetConnectionString("DBConnection1")))
            {
                ...
                var result = await connection.ExecuteAsync(ServiceCategoryQueries.Create_ServiceCategory, createServiceCategoryDTO, commandType: CommandType.StoredProcedure);
    ...
    public async Task<IReadOnlyList<ServiceCategory>> GetAllAsync(SearchRequest searchRequest)
    {
    ...
    public async Task<ServiceCategory> GetByIdAsync(long id)
    {
        using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, id))
        {
            ...
            {
                connection.Open();
                var result = await connection.QuerySingleOrDefaultAsync<ServiceCategory>(ServiceCategoryQueries.usp_GetByID_ServiceCategory, new { ID = id }, commandType: CommandType.StoredProcedure);
                (logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(result);
                return result;
            }
        }
