...
public class BankRepository : IBankRepository
{
    ...
    public BankRepository(IConfiguration configuration, IMapper mapper, ILogger<BankRepository> logger, IHttpContextAccessor httpContextAccessor)
    {
    ...
    public async Task<ListResponseWrapper<Bank>> GetAllAsync(SearchRequest searchRequest)
    {
        using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, searchRequest))
        {
            using (IDbConnection connection = new SqlConnection(configuration.GetConnectionString("DBConnection1")))
            {
				...
				var result = await connection.QueryAsync<Bank>(BankQueries.GetAll_Bank, parameters, commandType: CommandType.StoredProcedure);

				(logger as LoggingExtensions.MethodEntryExitLogger)?.SetResponse(result);
				var response = new ListResponseWrapper<Bank> { Data = result.ToList(), TotalCount = parameters.Get<int>("@TotalCount"), Code = parameters.Get<int>("@Code"), Message = parameters.Get<string>("@Message") }; return response;
			}
        }
    }
