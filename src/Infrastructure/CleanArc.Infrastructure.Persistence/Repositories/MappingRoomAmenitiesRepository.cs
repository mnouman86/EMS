...
namespace CleanArc.Infrastructure.Persistence.Repositories
{
    public class MappingRoomAmenitiesRepository : IMappingRoomAmenitiesRepository
    {
        ...
        public async Task<ResponseEntity> AddAsync(MappingRoomAmenities mappingRoomAmenities)
        {
            using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, mappingRoomAmenities))
            {
                using (IDbConnection connection = new SqlConnection(configuration.GetConnectionString("DBConnection1")))
                {
                    ...
                    var result = await connection.QueryFirstOrDefaultAsync<ResponseEntity>(MappingRoomAmenitiesQueries.Create_Mapping_RoomAmenities, createMappingRoomAmenities, commandType: CommandType.StoredProcedure);
        ...
        public Task<ListResponseWrapper<MappingRoomAmenities>> GetAllAsync(SearchRequest request)
        {
            throw new NotImplementedException();
        }

        public Task<SingleResponseWrapper<MappingRoomAmenities>> GetByIdAsync(SearchRequestById searchRequestById)
        {
            throw new NotImplementedException();
        }
