...
namespace CleanArc.Infrastructure.Persistence.Repositories
{
    public class CarImageRepository : ICarImageRepository
    {
        ...
        /// <inheritdoc/>
        public async Task<string> AddAsync(CarImage CarImage)
        {
            using (var logger = _logger.LogMethodEntryExit(_httpContextAccessor?.HttpContext, CarImage))
            {
                using (IDbConnection connection = new SqlConnection(configuration.GetConnectionString("DBConnection1")))
                {
                    ...
                    var result = await connection.ExecuteAsync(CarImageQueries.Create_CarImage, createCarImageDTO, commandType: CommandType.StoredProcedure);
        ...
        public Task<IReadOnlyList<CarImage>> GetAllAsync(SearchRequest request)
        {
            throw new NotImplementedException();
        }
